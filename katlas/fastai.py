...
# %% ../nbs/04_DL.ipynb 21
def train_dl(df, # dataframe contains features and targets
            splits, # tuple (train_index, valid_index)
            feat_col, # ['feat1', 'feat2', ...]
            target_col, # ['target1']
            model, # model function or class
            loss = mse, # loss function
            save = None, # name of the model to be solved, it will save to models/name.pth
            # lr=None, # learning rate
            epochs=4, # epochs to train,
             params = {} # params in fastai Learner
              ):
    ...
    # m = model(len(feat_col),len(target_col))
    ...
    learn.fit_one_cycle(epochs, **params)
...
def predict_dl(df, 
    ...
    if with_target:
        ...
        spearman_corr, _ = spearmanr(true.values.ravel(), preds.values.ravel())

        # Calculate the Pearson correlation coefficient
        pearson_corr, _ = pearsonr(true.values.ravel(), preds.values.ravel())
