...
# %% auto 0
__all__ = ['reduce_dim', 'get_pca', 'plot_cluster', 'plot_bar', 'plot_corr', 'kinase_report']
...
# %% ../nbs/02_plot.ipynb 6
def get_pca(df, # first column needs to be ID col, the rest are feature cols
            n,# n_components
            method = 'pca', # define demension reduction method: pca, umap, or tsne
            complexity = 15, # only work for umap and tsne; try 10-50 for umap
           ):
    # reduce dimension
    pca_df = reduce_dim(df,method = method, n_components=n, complexity = complexity)
    
    name = pca_df.columns[0]

    # add annotation
    pca_df.columns = f'{name}_' + pca_df.columns

    pca_df = pca_df.rename(columns = {f'{name}_{name}':f'{name}'})
