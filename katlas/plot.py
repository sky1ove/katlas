...
# %% auto 0
__all__ = ['set_sns', 'get_color_dict', 'get_hue_big', 'reduce_feature', 'plot_2d', 'plot_cluster', 'plot_bokeh', 'plot_rank',
           'plot_hist', 'plot_count', 'plot_bar', 'plot_group_bar', 'plot_box', 'plot_corr', 'get_similarity',
           'plot_matrix', 'get_AUCDF', 'plot_confusion_matrix']
...
def plot_2d(X: pd.DataFrame, # a dataframe that has first column to be x, and second column to be y
    ...
    sns.scatterplot(data=X, x=X.columns[0], y=X.columns[1], alpha=0.7, ax=ax, **kwargs)
    # have legend box on the right
    if 'hue' in kwargs: ax.legend(title=hue_title, loc='center left', bbox_to_anchor=(1.02, 0.5))
