...
# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/08_scoring.ipynb.

# %% auto #0
__all__ = ['EPSILON', 'multiply_23', 'multiply_20', 'cut_seq', 'get_dict', 'multiply', 'multiply_pspa', 'sumup',
           'duplicate_ref_zero', 'preprocess_ref', 'predict_kinase', 'Params', 'multiply_generic', 'predict_kinase_df',
           'get_pct', 'get_pct_df']

# %% ../nbs/08_scoring.ipynb #3870628d
...
# %% ../nbs/08_scoring.ipynb #4f41d685
EPSILON = 1e-8

# %% ../nbs/08_scoring.ipynb #803efd14
...
# %% ../nbs/08_scoring.ipynb #fc27f20a
multiply_23 = partial(multiply,num_aa=23)

# %% ../nbs/08_scoring.ipynb #e7b891e1
multiply_20 = partial(multiply,num_aa=20)

# %% ../nbs/08_scoring.ipynb #e118db0f
...
def predict_kinase(input_string: str, # site sequence
    ...
    parsed_keys = get_dict(input_string)
    ref_key_set = set(ref.columns)
    candidate_keys = [key for key in parsed_keys if key in ref_key_set]

    results = []

    for kinase, row in ref.iterrows():
        r_dict = row.dropna().to_dict()
        pos_aa_val = [r_dict[key] for key in candidate_keys if key in r_dict]
        score = func(pos_aa_val, kinase)
        results.append(score)

    if verbose:
        print(f"considering string: {candidate_keys}")

    out = pd.Series(results, index=ref.index).sort_values(ascending=False)
    return out.round(3).dropna()

# %% ../nbs/08_scoring.ipynb #84339a93
