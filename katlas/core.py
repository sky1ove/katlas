...
class Data:
    ...
    OCHOA_URL = "https://github.com/sky1ove/katlas/raw/main/database/ochoa_sites.parquet"
...
def extract_site_seq(df,sequence_col,position_col):
    data = []
    for i, r in tqdm(df.iterrows(),total=len(df)):
        ...
        subseq = r[sequence_col][max(0, start):min(len(r[sequence_col]), end)]

        # Pad the subsequence if needed
        if start < 0:
            subseq = "_" * abs(start) + subseq
        if end > len(r[sequence_col]):
            subseq = subseq + "_" * (end - len(r[sequence_col]))

        data.append(subseq)
    return np.array(data)

# %% ../nbs/00_core.ipynb 14
def agg_seq(df,agg_col = None):
...
def get_dict(s):
    "Get dictionary of input string; no need for the star in the middle; make sure it is 15 length"
...
def predict_kinase(df, ref, seq_col, seq_id):
    results=[]
